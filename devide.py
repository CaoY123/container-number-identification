    ...
    for image_dir in os.listdir(single_image_dir):
        ...
        for image_path in os.listdir(image_dir):
            ...
            if image_path.endswith('.jpg'):
                ...
                # 裁剪出字符部分
                img_cropped = img[
                              np.where(row_white_pixel_count > 0)[0][0]:np.where(row_white_pixel_count > 0)[0][-1] + 1,
                              np.where(col_white_pixel_count > 0)[0][0]:np.where(col_white_pixel_count > 0)[0][-1] + 1]
