...
def process_image(opt):
    ...
    # 数组vertical_projection、horizontal_projection的每个像素值为沿鸽子方向上的黑色像素的数量
    # 计算垂直投影，是一个一维数组，其反映了从上到下像素分布的情况，大小为从左到右的像素数
    vertical_projection = projection(255 - binary_img, axis=0)
    # 计算水平投影，是一个一维数组，其反映了从左到右像素分布的情况，大小为从上到下的像素数
    horizontal_projection = projection(255 - binary_img, axis=1)
    ...
    for line in lines:
        # 对每行进行垂直投影
        line_projection = projection(255 - line, axis=0)
        # 计算每行的垂直谷值
    ...
    for image_dir in os.listdir(single_image_dir):
        ...
        for image_path in os.listdir(image_dir):
            ...
            if image_path.endswith('.jpg'):
                ...
                # 统计每行水平方向上的黑色像素数
                row_black_pixel_count = np.sum(img == 0, axis=1)

                # 统计每列垂直方向上的黑色像素数
                col_black_pixel_count = np.sum(img == 0, axis=0)

                # 裁剪出字符部分
                img_cropped = img[
                              np.where(row_black_pixel_count > 0)[0][0]:np.where(row_black_pixel_count > 0)[0][-1] + 1,
                              np.where(col_black_pixel_count > 0)[0][0]:np.where(col_black_pixel_count > 0)[0][-1] + 1]

                # 调整图像大小为 NORMAL_IMAGE_SIZE(x * y)
                img_resized = cv2.resize(img_cropped, NORMAL_IMAGE_SIZE, interpolation=cv2.INTER_AREA)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', type=str, default='binary_images/IMG_0180_0.jpg', help='picture file')
