...
def run_pretreat(opt):
    ...
    # 使用自适应阈值处理
    adaptive_thresh_x = cv2.adaptiveThreshold(scaled_sobel_x, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11,
                                              2)
    adaptive_thresh_y = cv2.adaptiveThreshold(scaled_sobel_y, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11,
                                              2)

    # 将x和y方向的边缘检测结果组合起来
    combined = cv2.bitwise_or(adaptive_thresh_x, adaptive_thresh_y)

    # 3. 图像腐蚀
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    eroded_img = cv2.erode(combined, kernel)
    ...
    writeResult = cv2.imwrite(filename, closing)
    if writeResult:
